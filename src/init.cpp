...
#include "config/dogecoinprivate-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("dogecoinprivate-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "dogecoinprivate.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "dogecoinprivated.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the DOGP and zDOGP money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 31472, 31474));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in DOGP/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in DOGP/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, dogecoinprivate, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in DOGP/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all DOGP specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-dogpstake=<n>", strprintf(_("Enable or disable staking functionality for DOGP inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zdogpstake=<n>", strprintf(_("Enable or disable staking functionality for zDOGP inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzdogp=<n>", strprintf(_("Enable automatic wallet backups triggered after each zDOGP minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zdogpbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zDOGP backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizedogecoinprivateamount=<n>     " + strprintf(_("Keep N DOGP anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The Dogecoin Private Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("dogecoinprivate-loadblk");
...
/** Sanity checks
 *  Ensure that DOGP is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Dogecoin Private Core is shutting down."));
...
#endif
    // Make sure only a single DOGP process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Dogecoin Private Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //DOGP: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //DOGP specific: zerocoin and spork DB's
                ...
                // DOGP: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculatezDOGPMinted();
                        RecalculatezDOGPSpent();
                    }
                    RecalculateDOGPSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // DOGP: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Dogecoin Private Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Dogecoin Private Core to complete") << "\n";
        ...
        zwalletMain = new CzDOGPWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zDOGPWallet
        uiInterface.InitMessage(_("Syncing zDOGP wallet..."));

        bool fEnableZPivBackups = GetBoolArg("-backupzdogp", true);
        pwalletMain->setZPivAutoBackups(fEnableZPivBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zdogpTracker->Init();
...
//    }
//
//    nAnonymizeBitCoinOneAmount = GetArg("-anonymizedogecoinprivateamount", 0);
...
//    if (nAnonymizeBitCoinOneAmount < 2) nAnonymizeBitCoinOneAmount = 2;
    ...
    LogPrintf("Anonymize DOGP Amount %d\n", nAnonymizeBitCoinOneAmount);
    ...
    /* Denominations
       ...
       For example:
       1DOGP+1000 == (.1DOGP+100)*10
       10DOGP+10000 == (1DOGP+1000)*10
    */
