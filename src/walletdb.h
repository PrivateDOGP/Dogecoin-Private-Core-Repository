...
public:
    ...
    bool WritezDOGPSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadzDOGPSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadzDOGPSeed_deprecated(uint256& seed);
    bool ErasezDOGPSeed();
    bool ErasezDOGPSeed_deprecated();

    bool WritezDOGPCount(const uint32_t& nCount);
    bool ReadzDOGPCount(uint32_t& nCount);
