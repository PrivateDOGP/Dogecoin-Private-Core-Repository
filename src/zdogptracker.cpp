...
CzDOGPTracker::CzDOGPTracker(std::string strWalletFile)
{
...
CzDOGPTracker::~CzDOGPTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzDOGPTracker::Init()
{
...
bool CzDOGPTracker::Archive(CMintMeta& meta)
{
...
bool CzDOGPTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzDOGPTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzDOGPTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzDOGPTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzDOGPTracker::GetSerialHashes()
{
...
CAmount CzDOGPTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzDOGPTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzDOGPTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzDOGPTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzDOGPTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzDOGPTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzDOGPTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzDOGPTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzDOGPTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzDOGPTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzDOGPTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDOGPTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDOGPTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzDOGPTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzDOGPTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzDOGPTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzDOGPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzdogp from DB\n", __func__, listDeterministicDB.size());
    }
