...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zdogpTracker = unique_ptr<CzDOGPTracker>(new CzDOGPTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == zDOGP_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == zDOGP_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
