...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered dogecoinprivate address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 DOGP.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal dogecoinprivate:URI (dogecoinprivate:XyZ...?message=example)
            {
...
static void NotifyzDOGPReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzDOGPReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzDOGPReset.connect(boost::bind(NotifyzDOGPReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzDOGPReset.disconnect(boost::bind(NotifyzDOGPReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zdogpTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
