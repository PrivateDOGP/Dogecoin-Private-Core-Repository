...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zdogpTracker->HasMintTx(hash))) {
            //zDOGP stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakezDOGP;
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime);
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zDOGP;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
...
bool IszDOGPType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakezDOGP:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zDOGP:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakezDOGP || type == TransactionRecord::MNReward) {
