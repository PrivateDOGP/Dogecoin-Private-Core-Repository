...
CzDOGPWallet::CzDOGPWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zdogp seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzDOGPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezDOGPSeed_deprecated()) {
                    LogPrintf("%s: Updated zDOGP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zdogp seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zdogp wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDOGPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezDOGPCount(nCountLastUsed);
    else if (!walletdb.ReadzDOGPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDOGPWallet::Lock()
{
    seedMaster = 0;
}

void CzDOGPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDOGPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozDOGP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDOGPWallet::LoadMintPoolFromDB()
{
...
void CzDOGPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDOGPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDOGPWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zdogpTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDOGPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozDOGP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zdogpTracker which also adds to database
    pwalletMain->zdogpTracker->Add(dMint, true);
...
void CzDOGPWallet::SeedTozDOGP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDOGPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDOGPWallet::GenerateDeterministiczDOGP(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDOGPWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDOGPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
